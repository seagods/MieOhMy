...
int main(){
    ...
    double C_Cstar[nvec],D_Dstar[nvec]; 
    ...
    int numx=1000;
    FILE *fp1;
    fp1=fopen("CCstar.dat","w");
    fprintf(fp1,"%d  \n", 1) ;
    fprintf(fp1,"%d  \n", numx); 
    FILE *fp2;
    fp2=fopen("DDstar.dat","w");
    fprintf(fp2,"%d  \n", 1) ;
    fprintf(fp2,"%d  \n", numx);  
    ...
    for(int ix=0; ix<numx; ix++){
      ...
      DaveCD(C,D,an,bn,istop,istop1,zero);
    
      for(int i=1; i < istop1; i++){
          for(int j=i; j < istop1; j++){
           C_Cstar[j+(i-1)*istop-i*(i-1)/2-1]=real(C[i-1]*conj(C[j-1]));
           if(ix==500)cout << "index=" << j+(i-1)*istop-i*(i-1)/2  << "   " << 
           C_Cstar[j+(i-1)*istop-i*(i-1)/2-1] << "  "<<  C[i-1] << "  " << C[j-1] << endl;
           D_Dstar[j+(i-1)*istop-i*(i-1)/2-1]=real(D[i-1]*conj(D[j-1]));
        } 
       } 
      int k=5;
      fprintf(fp1," %f %f \n",ex,C_Cstar[k] );
